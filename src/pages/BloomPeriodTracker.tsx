...
import { Calendar, Heart, Droplets, Moon, Star, TrendingUp, Crown, Lock, Plus, ChevronLeft, ChevronRight, Baby, Activity, Brain, Apple, MessageCircle, Send } from "lucide-react";
...
const BloomPeriodTracker = () => {
  ...
  const handleSymptomToggle = (symptomId: string) => {
    if (!isLoggedIn) {
      alert('Please log in to track symptoms');
      return;
    }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center space-x-4">
              ...
              {!isLoggedIn ? (
                <Button onClick={() => setIsLoggedIn(true)} className="bg-purple-600 hover:bg-purple-700">
                  Login
                </Button>
              ) : (
            ...
            <Card className="border-pink-200">
              <CardHeader>
                <CardTitle className="text-lg font-bold text-gray-900">
                  Today's Log {!isLoggedIn && <Lock className="w-4 h-4 inline ml-2 text-gray-400" />}
                </CardTitle>
              </CardHeader>
              <CardContent>
                <Tabs defaultValue="symptoms" className="w-full">
                        ...
                        <div className="flex space-x-2">
                          {['Light', 'Medium', 'Heavy', 'Spotting'].map(intensity => (
                            <Button
                              ...
                              onClick={() => isLoggedIn ? setFlowIntensity(intensity) : alert('Please log in to track flow')}
                              disabled={!isLoggedIn}
                            >
                              {intensity}
                            </Button>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {moodOptions.slice(0, isPremium ? 10 : 6).map(mood => (
                          <Button
                            ...
                            size="sm"
                            className="text-xs justify-start h-8"
                            onClick={() => isLoggedIn ? setTodayMood(mood.id) : alert('Please log in to track mood')}
                            disabled={!isLoggedIn}
                          >
                            {mood.icon} {mood.name}
                          </Button>
                  ...
                  <TabsContent value="notes" className="space-y-4">
                    <div>
                      ...
                      <Textarea
                        id="notes"
                        placeholder={isLoggedIn ? "How are you feeling today? Any observations about your cycle, symptoms, or general wellbeing?" : "Please log in to add personal notes"}
                        value={todayNotes}
                ...
                <Button 
                  ...
                  disabled={!isLoggedIn}
                >
                  {isLoggedIn ? "Save Today's Data" : "Login to Save Data"}
                </Button>
              </CardContent>
            </Card>
